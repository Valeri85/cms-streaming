 ...
 * ALL 50 LANGUAGE PRESETS PRESERVED
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_language'])) {
    ...
    if (empty($langCode)) {
    ...
    } elseif (file_exists(LANG_DIR . $langCode . '.json')) {
        ...
        if (!empty($copyFrom) && file_exists(LANG_DIR . $copyFrom . '.json')) {
            $sourceData = json_decode(file_get_contents(LANG_DIR . $copyFrom . '.json'), true);
            if ($sourceData) {
        ...
        $newLangFile = LANG_DIR . $langCode . '.json';
...
<style>
    .add-language-container { max-width: 900px; }
    ...
    .presets-grid::-webkit-scrollbar { width: 8px; }
    .presets-grid::-webkit-scrollbar-track { background: #e9ecef; border-radius: 4px; }
    .presets-grid::-webkit-scrollbar-thumb { background: #667eea; border-radius: 4px; }
    ...
    .preset-btn:hover { border-color: #3498db; background: #e3f2fd; }
    .preset-btn.selected { border-color: #27ae60; background: #e8f5e9; }
    
    .preset-flag { font-size: 24px; }
    .preset-info { flex: 1; min-width: 0; }
    .preset-name { font-weight: 600; color: #2c3e50; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .preset-code { font-size: 11px; color: #7f8c8d; }
    ...
    .manual-input-section h3 { margin: 0 0 20px 0; color: #2c3e50; font-size: 16px; }
    ...
    .copy-from-section h4 { margin: 0 0 15px 0; color: #1565c0; }
    ...
    .copy-option:hover { border-color: #2196f3; }
    .copy-option.selected { border-color: #2196f3; background: #e3f2fd; }
    .copy-option input { margin: 0; }
    .copy-flag { font-size: 20px; }
    .copy-name { flex: 1; font-weight: 500; }
        ...
        <form method="POST" id="addLanguageForm">
            ...
            <div class="info-box info" style="margin-bottom: 25px;">
                ...
                <div class="info-box-content">
                    ...
                    The language will be immediately available to assign to websites. You'll be redirected to the translation editor.
                </div>
            </div>
